...
export default function Kurta() {
  ...
  // Admin create kurta popup states
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setKurtas(kurtas.filter((k) => k._id !== id));
    } catch (err) {
      alert(err.message);
    }
  };
  ...
  const handleAddToCart = async (kurta) => {
    ...
    try {
      ...
      alert("Kurta added to cart!");
      await refreshUser(); // update navbar cart count
    } catch (err) {
      alert(err.message);
    }
  };
  ...
  if (loading) return <p className="text-center mt-10">Loading kurtas...</p>;
  if (error) return <p className="text-center mt-10 text-red-600">{error}</p>;

  return (
    <div className="relative max-w-7xl mx-auto px-6 py-10">
      {/* ---------- Create Kurta Popup Form (Admin Only) ---------- */}
      {isAdmin && showForm && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50">
          ...
          >
            <h2 className="text-xl font-semibold mb-4">Create Kurta</h2>
      ...
      {/* ---------- Kurta Cards ---------- */}
      {kurtas.length === 0 ? (
        <p className="text-center text-gray-600 mt-10">No kurtas available</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {kurtas.map((kurta) => (
            ...
            >
              <img
                src={kurta.image?.url || "/placeholder.png"}
                alt="Kurta"
                className="w-full h-64 object-cover rounded-md mb-4"
              />

              <p className="font-semibold text-lg">₹{kurta.price}</p>
              <p className="text-gray-600">{kurta.notes || "No notes"}</p>
              ...
              {isAdmin && (
                <button
                  onClick={() => handleDelete(kurta._id)}
