...
export default function Kurta() {
  ...
  const handleCreateKurta = async (e) => {
    e.preventDefault();
    if (!isAdmin) return alert("Only admins can create kurtas.");
    setUploading(true);
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || "Failed to create kurta");

      setItems((prev) => [...prev, data.kurta]);
      setTitle("");
      setNotes("");
      setPrice("");
      setImageFile(null);
  ...
  const handleDelete = async (id) => {
    if (!isAdmin) return;
    if (!window.confirm("Delete this kurta?")) return;
    try {
    ...
    <div className="bg-gray-50 py-16 px-4">
      <div className="max-w-7xl mx-auto space-y-32">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          <h1 className="text-4xl font-extrabold text-purple-900">Kurta Collection</h1>
          {isAdmin && (
            <button
        ...
        {/* Loading / Empty */}
        {loading ? (
          <p className="text-center text-gray-600">Loading kurtas...</p>
        ) : items.length === 0 ? (
          <p className="text-center text-gray-500">No kurtas available.</p>
        ) : (
          items.map((it, idx) => (
            <section
              key={it._id}
            ...
            >
              ...
              <div className="w-full lg:w-1/2 space-y-4">
                ...
                <p className="text-gray-700">{it.notes}</p>
          ...
          >
            ...
            {imagePreview && (
              <img
                src={imagePreview}
                alt="Preview"
                className="mt-3 w-full h-44 object-cover rounded-lg shadow-md"
              />
            )}
            <div className="flex justify-end gap-3 mt-4">
              ...
              <button
                type="submit"
                className="px-4 py-2 bg-purple-900 text-white rounded-lg hover:bg-purple-800 transition"
              >
      ...
      {showBookingModal && bookingProduct && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center px-4 z-50">
          ...
          >
            ...
            <p className="text-gray-600">Booking for <strong>{bookingProduct.title}</strong></p>
