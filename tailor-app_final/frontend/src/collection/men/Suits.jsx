...
export default function Suit() {
  ...
  const PLACEHOLDER = "/placeholder_suit.png";
  ...
  useEffect(() => {
    ...
    const fetchItems = async () => {
      try {
        ...
        if (!res.ok) throw new Error(data.message || "Failed to fetch suits");
        if (mounted) setItems(data.suits || []);
      } catch (err) {
  ...
  const handleCreateSection = async (e) => {
    e.preventDefault();
    if (!isAdmin) return alert("Only admins can create suits.");
    setUploading(true);
  ...
  const handleDelete = async (id) => {
    if (!isAdmin) return;
    if (!window.confirm("Delete this suit?")) return;
    ...
    <div className="bg-gray-50 py-16 px-4">
      <div className="max-w-7xl mx-auto space-y-32">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          <h1 className="text-4xl font-extrabold text-purple-900">Suit Collection</h1>
          {isAdmin && (
            <button
        ...
        {/* Loading / Empty */}
        {loading ? (
          <p className="text-center text-gray-500">Loading suits...</p>
        ) : items.length === 0 ? (
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center px-4 z-50">
          <form className="bg-white rounded-3xl p-6 w-full max-w-md shadow-2xl space-y-4" onSubmit={handleCreateSection}>
            <div className="flex justify-between items-center">
              <h3 className="text-xl font-bold">Add New Suit</h3>
              <button type="button" onClick={() => setShowAdminForm(false)}>✕</button>
            </div>
            <input
              type="text"
              placeholder="Suit Title"
              value={title}
